...
class WorkOrderMasterController extends ResourceController
{
    ...
    public function updateWorkOrderMaster($id)
    {
        ...
        $existing = $this->workOrderMasterModel->find($id);
        if (!$existing) {
            return $this->respond([
                'status'  => false,
        ...
        // Dynamic validation for work_order_db uniqueness (ignore current ID)
        $rules = $this->workOrderMasterModel->getValidationRules();
        if (isset($data['work_order_db'])) {
            // required|string|max_length[50]|is_unique[work_order_master.work_order_db]
            $rules['work_order_db'] = 'required|string|max_length[9]|is_unique[work_order_master.work_order_db ,id,' . intval($id) . ']';
        }

        $this->workOrderMasterModel->setValidationRules($rules);
